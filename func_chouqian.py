...
class Chouqian(object):
    def __init__(self) -> None:
        self.df = pd.read_csv("chouqian.csv", delimiter="\t")
        self.cys, self.zis, self.yins = self._build_data()

    def _build_data(self):
        df = self.df.copy()
        df["shouzi"] = df["chouqian"].apply(lambda x: x[0])
        df["mozi"] = df["chouqian"].apply(lambda x: x[-1])

        df["shouyin"] = df["pingyin"].apply(lambda x: x.split(" ")[0])
        df["moyin"] = df["pingyin"].apply(lambda x: x.split(" ")[-1])

        cys = dict(zip(df["chouqian"], df["moyin"]))
        zis = df.groupby("shouzi").agg({"chouqian": set})["chouqian"].to_dict()
        yins = df.groupby("shouyin").agg({"chouqian": set})["chouqian"].to_dict()

        return cys, zis, yins

    def isChouqian(self, cy: str) -> bool:
    ...
    def getMeaning(self, cy: str) -> str:
        ress = self.df[self.df["chouqian"] == cy].to_dict(orient="records")
        if ress:
            res = ress[0]
            rsp = res["chouqian"] + "\n" + res["pingyin"] + "\n" + res["jieshi"]
